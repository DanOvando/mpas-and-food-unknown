...
k_plot <- k_series %>% 
  ...
  scale_x_continuous(labels = percent, name = "% of K in MPA") + 
  scale_y_continuous(name = "Change in Global Yield (MMT)") + 
  geom_label_repel(data = k_peak, aes(percent_k, yield_effect, label = round(yield_effect,2)),size = 4,box.padding = 4, show.legend = FALSE, fill = "darkslategray", min.segment.length = 0) + 
  labs(title = "(a)") + 
...
space_plot <- k_series %>% 
  ...
  scale_y_continuous(name = "") + 
  geom_label_repel(data = k_peak, aes(rank / 100, yield_effect, label = round(yield_effect,2)),size = 4, show.legend = FALSE, fill = "darkslategray", min.segment.length = 0) + labs(title = "(b)") + 
...
yield_curve_plot <- (k_plot / space_plot) &  (theme_cowplot(font_family = "Helvetica") + theme(legend.position = "top"))
